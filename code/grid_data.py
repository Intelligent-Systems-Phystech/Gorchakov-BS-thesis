...
def get_linear_constraints(grid_name, check_pp_vs_new_form=False):
    ...
    # for name in left_ineqs:
    # print(A[:, k1_Va:kN_Va].shape)
    # print(A_Va_inv_A_Pg.shape)
    # print(np.dot(A[:, k1_Va:kN_Va].todense(), A_Va_inv_A_Pg).shape)
    ...
    if check_pp_vs_new_form:
        res_hands_new = optim.linprog(
            ...
            A_full_new,
            u_full_new,
            x0=x0_pp[k1_Pg + 1 : kN_Pg],
            method="simplex",
            options={"tol": 0.9e-3},
        )
        # sanity check continuation
        all_gens_restorated = np.array(
            [(ul_ref_eq - A_ref_eq[1:].dot(res_hands_new.x)) / A_ref_eq[0]]
            + list(res_hands_new.x)
        )
        print("sol from pandapower proximity:", x[k1_Pg:kN_Pg] - all_gens_restorated)

        print(
            "cost from this method:",
            res_hands_new.fun + CC[k1_Pg] * ul_ref_eq / A_ref_eq[0],
        )
